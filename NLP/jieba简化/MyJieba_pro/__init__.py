...
# 通用词库
DICT = get_abs_path(BASE_PATH, 'dict.txt')  # jieba原词典
NA = 'NA'
...
class Io:
    """读文件"""
    @staticmethod
    def txt2df(filename=DICT, sep=' ', names=None):
...
class Cutter:
    ...
    def _cut_hmm(self, sentence):
        ...
        if buf:
            if len(buf) == 1:
                yield buf
            elif not self.dt.get(buf, [0, NA])[0]:
                recognized = hmm.cut(buf)
                for t in recognized:
                    yield t
            else:
...
cut_hmm = lambda sentence: cutter().cut_hmm(sentence)
lcut_hmm = lambda sentence: list(cut_hmm(sentence))
